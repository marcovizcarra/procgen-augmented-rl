...
def _extract_q_state_dict(ckpt: dict) -> dict:
    ...
    if "q_state_dict" in ckpt and isinstance(ckpt["q_state_dict"], dict):
        return ckpt["q_state_dict"]
    if "state_dict" in ckpt and isinstance(ckpt["state_dict"], dict):
        sd = ckpt["state_dict"]
        if any(k.startswith("enc.") or k.startswith("head.") for k in sd.keys()):
            return sd
    raise KeyError("Could not find CQL Q state_dict in checkpoint.")


def build_q_for_checkpoint(ckpt: dict, device: torch.device) -> nn.Module:
    sd = _extract_q_state_dict(ckpt)
    ...
    model = DiscreteQ(n_actions=n_actions, hidden=hidden).to(device)
...
def run_eval(env, qnet, device, episodes: int, desc: str = "", progress: bool = True, print_every: int = 25):
    returns = []
    obs = reset_env(env)
    ep_ret = 0.0
    total_steps = 0
